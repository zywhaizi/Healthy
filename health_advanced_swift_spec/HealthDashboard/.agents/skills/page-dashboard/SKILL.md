...
HDDashboardViewController   ← UIViewController，持有 ViewModel
  └── HDDashboardViewModel  ← ObservableObject，@Published 属性驱动 UI
        └── HDHealthDataModel.shared()  ← 只读访问
```

## ViewModel 数据绑定

```swift
// ViewModel 通过 refreshData() 读取 Model 数据
func refreshData() {
...
```swift
let dash = HDDashboardViewController()
```
...
**sleepHours 类型**：`NSArray<NSNumber *>` 在 Swift 侧直接赋值给 `[NSNumber]`，无需 `as?` 转型

**moodRecords 类型**：`NSArray<HDMoodRecord *>` 在 Swift 侧直接赋值给 `[HDMoodRecord]`
